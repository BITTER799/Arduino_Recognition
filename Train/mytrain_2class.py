...
# 设置训练参数
nb_train_samples =  24908  # 训练样本数
nb_validation_samples = 6159  # 测试样本数
nb_epoch =30  # 训练轮数
batch_size = 64 # 批次大小
...
train_flow = train_pic_gen.flow_from_directory(
    ...
    color_mode='rgb',
    classes=[str(i) for i in range(1, 3, 1)],
...
validation_flow = validation_pic_gen.flow_from_directory(
    ...
    #color_mode='grayscale',  # 输入图片为灰度图片
    color_mode='rgb',
    classes=[str(i) for i in range(1, 3, 1)],  # 标签
...
# 回调函数，保存最佳训练参数
checkpointer = ModelCheckpoint(filepath="./weights/weights_2class.h5", monitor='val_acc',verbose=1, save_best_only=True,mode='max')

# 导入上次训练的权重
try:
    model.load_weights('./weights/weights_2class.h5')
    print("load weights...")
except:
    print("not weights")
    pass
