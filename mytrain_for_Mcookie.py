...
# 设置训练参数
nb_train_samples =  120601  # 训练样本数
nb_validation_samples = 15840  # 测试样本数
nb_epoch =100  # 训练轮数
batch_size = 64 # 批次大小
...
# 训练和测试数据路径
target = './data/'
train_data_dir = target + 'data_light'
validation_data_dir = target + 'validation_process'
...
train_flow = train_pic_gen.flow_from_directory(
    ...
    color_mode='rgb',
    classes=[str(i) for i in range(1, 13, 1)],
...
validation_flow = validation_pic_gen.flow_from_directory(
    ...
    #color_mode='grayscale',  # 输入图片为灰度图片
    color_mode='rgb',
    classes=[str(i) for i in range(1, 13, 1)],  # 标签
...
checkpointer2 = ModelCheckpoint(filepath="./weights/weights_mcookie_12.h5", monitor='val_acc',verbose=1, save_best_only=True)

# 导入上次训练的权重
try:
    model.load_weights('./weights/weights_mcookie_12.h5')
    print("load weights...")
except:
    print("not weights")
    pass
